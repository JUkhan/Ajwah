...
import { AUTO_FOCUS, CHECK_ERROR, REFRESH, toSingleKey, toMultiKeys } from './utility';
import { Actions } from './actions';
import {Dispatcher} from './dispatcher';
import {Action} from './action';

export class StateObserver extends BehaviorSubject<ObserverState>{
    ...
    select(fieldName: string) {
        return this.pipe(pluck('fields', fieldName), filter(el => typeof el !== 'undefined'), distinctUntilChanged());
    }
    emit(fieldName: string, value: any) {
        this.next({ errors: this.value.errors, fields: { ...this.value.fields, [fieldName]: value } })
    }
    emitError(key: string, value: any) {

        this.next({ fields: this.value.fields, errors: { ...this.value.errors, [key]: value } })
    }
    selectError(fieldName: string) {
        return this.pipe(pluck('errors', fieldName), distinctUntilChanged());
    }
    get hasError$() {
        return this.pipe(map(state => state.errors), map(errors => {
            return Object.entries(errors).filter(([key, value]) => value).length > 0;
        }))
    }
    ...
    reset() {
        Object.keys(this.__state).forEach(key => this.notify(key, { value: '', error: '', hasError: false, loading: false, flag: false }));

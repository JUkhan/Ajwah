...
export class ProductController extends StateController<ProductStateState> {
    ...
    effects(){
        ...
        this.registerEffect(
            actions$.whereTypes('selectDeparment', 'selectCategory')
            .pipe(
                map(async action=>{
                    const {selectedDepartment} =await this.remoteState<DepartmentState>('department');
                    return `product/inDepartment/${selectedDepartment}`;
                }),
                mergeMap(path=>from(path)),
                map(path=>({type:'setProduct', payload:{path, products:[{name:'pro2', id:2},
                {name:'pro3', id:3},
                {name:'pro4', id:4}]}}))
            )
        )
